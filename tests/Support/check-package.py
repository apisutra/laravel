...
required = ['composer.json', 'README.md', 'CHANGELOG.md', 'LICENSE', 'src/SdkServiceProvider.php',
            'src/Contracts/RequestFactoryInterface.php', 'docs/en/README.md', 'docs/ru/README.md', 'docs/translations.json',
            'resources/lang/en/queue.php', 'resources/lang/ru/queue.php']
...
with tempfile.TemporaryDirectory(prefix='apisutra-dist-') as temporary:
    ...
    core = folder / 'core'
    core.mkdir()
    run(['composer', 'archive', '--format=tar', f'--dir={folder}', '--file=core', '--no-interaction'], ROOT / '.test/core')
    with tarfile.open(folder / 'core.tar') as archive:
    ...
    for kind in ['git', 'composer']:
        ...
        run(['python3', str(ROOT / 'tests/Support/check-docs.py'), '--root', str(checkout), '--distribution'])
        run(['composer', 'validate', '--strict'], checkout)
        print(f'{kind}: installing Laravel SDK without dev dependencies', flush=True)
        sdk_report = folder / f'{kind}-sdk.json'
        run(['python3', str(ROOT / 'tests/Support/check-sdk-package.py'), '--root', str(checkout), '--report', str(sdk_report), '--core', str(core)])
        report['archives'][kind] = {'bytes': archive_path.stat().st_size,
                                    'files': len(contents[kind]),
...
print('Git и Composer dist совпадают; Laravel-установки без dev-зависимостей прошли.')
